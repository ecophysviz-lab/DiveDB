...
This page is a comprehensive, token-efficient technical reference derived from
``DiveDB/AI_DOCS.md``. It covers architecture, module interfaces, data structures,
integration points, common patterns, and maintenance guidelines.
...
.. list-table::
   ...
   * - **Project**
     - Apache Iceberg data lake with Notion metadata integration for biologging data
   * - **Entry Point**
     - ``DuckPond.from_environment()`` (``duck_pond.py``:384)
   * - **Tech Stack**
     - DuckDB, Apache Iceberg, PyArrow, xarray, Notion API, requests
   * - **Key Dependencies**
     - ``NotionORMManager``, ``ImmichService``, ``DataUploader``
   * - **Config**
     - Environment variables (``ICEBERG_PATH``, S3 config, Notion credentials)

Terminology
-----------

See the dedicated :doc:`terminology` page for the full organism/animal migration
reference table and rules for new code.
...
.. list-table::
   ...
   * - File
     ...
     - Lines
   * - ``duck_pond.py``
     - Main data lake interface
     - ``DuckPond``
     - 1691
   * - ``notion_orm.py``
     - Notion database ORM
     - ``NotionORMManager``, ``NotionModel``
     - 512
   * - ``data_uploader.py``
     - NetCDF upload and validation
     - ``DataUploader``, ``NetCDFValidationError``
     - 705
   * - ``dive_data.py``
   ...
   * - ``utils/netcdf_conversions.py``
     - NetCDF conversion helpers
     - ``matlab_datenum_to_datetime_vectorized()``, ``infer_date_format()``
     - 146
   * - ``utils/openstack.py``
...
**Key Classes**: ``DuckPond`` — primary interface for data operations.

**Key Methods**:

- ``from_environment(**kwargs)`` → ``DuckPond`` — Factory method from env vars
- ``get_data(dataset, labels, organism_ids, deployment_ids, recording_ids, groups, classes, date_range, frequency, limit, pivoted, apply_timezone_offset, add_timestamp_column, use_cache)`` → ``pd.DataFrame | DiveData`` — Query data with optional resampling and caching; ``animal_ids`` accepted as deprecated kwarg
...
- ``write_event(dataset, deployment, animal, event_key, datetime_start, datetime_end, recording, group, short_description, long_description, event_data)`` → ``None`` — Write single event (convenience for Dash UI)
...
- ``get_3d_model_for_organism(organism_id, use_cache)`` → ``Dict`` — Fetch 3D model info from Notion; ``get_3d_model_for_animal`` is a deprecated alias
- ``_get_organism_icons(organism_ids)`` → ``Dict[str, str]`` — Fetch icon URLs; ``_get_animal_icons`` delegates to this
...
title, rich_text, number, select, multi_select, date, relation, checkbox, url,
email, phone_number, formula, files.

**Relationship Methods**:
Auto-injected ``get_{related_db}()`` methods for relation properties. Traverses
Notion relations to load related records.

**Database Map**:
Maps database names to Notion database IDs. Common databases: Deployment DB,
Recording DB, Logger DB, Organism DB (falls back to Animal DB), Asset DB, Dataset DB,
Signal DB, Standardized Channel DB. ``get_model("Animal")`` tries ``"Organism"``
first, falls back to ``"Animal"`` for older Notion workspaces.
...
**Key Classes**: ``DiveData`` — Query results wrapper.
...
**Key Classes**: ``ImmichService`` — Media asset service.
...
- ``prepare_video_options_for_react(media_result, expires_hours)`` → ``Dict`` — Format videos for React
...
**Return Format**: All methods return ``{"success": bool, "data": ..., "error": ...}`` dicts.
...
**Key Classes**: ``WarehouseConfig`` — Configuration dataclass.
...
**Backend Selection**: S3 if all S3 env vars present; local filesystem otherwise.
...
**Key Classes**: ``CatalogManager`` — Catalog manager.
...
**Key Classes**: ``DuckDBConnection`` — Connection manager.
...
**Key Classes**: ``DatasetManager`` — Dataset manager.
...
**Key Classes**: ``NotionIntegration`` — Notion loader.
...
**Parallel Loading**: Uses ``concurrent.futures`` for parallel Notion API calls.
...
**Key Classes**: ``SwiftClient`` — Swift client wrapper.

**Environment Variables**: ``OPENSTACK_AUTH_URL``, ``OPENSTACK_APPLICATION_CREDENTIAL_ID``,
...
**Key Classes**: ``OpenStackStorage`` — OpenStack storage interface.
...
**Key Classes**: ``TimingContext`` — Context manager for timing operations.
...
.. code-block:: python
    ...
    {
        "deployment": str,
        "organism_id": str,   # canonical Python key; "animal" preserved in Iceberg column
        "deployment_date": str,
        "min_date": str,
        "max_date": str,
        "sample_count": int,
